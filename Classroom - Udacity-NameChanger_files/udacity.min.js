...
angular.module("udacity.overview",["udacity.core","youtube"]);angular.module("udacity.courseCatalog",["udacity.core"]);angular.module("udacity.editor",["udacity.core","services.editor","directives.editor","controllers.editor","services.lesson"]);angular.module("udacity.settings",["udacity.core","controllers.settings","directives.location"]);angular.module("udacity.payment",["udacity.core"]);angular.module("udacity.core").config(["$locationProvider",function(a){a.html5Mode(!1);a.hashPrefix("!")}]);
angular.module("udacity.core").config(function(a){a.decorator("$exceptionHandler",["$delegate",function(a){return function(b,c){Raygun&&Raygun.send(b);a(b,c)}}])})})();angular.module("core","foundation gateways user utility filters widgets ngRoute ngSanitize ngAnimate payment embedded".split(" "));angular.module("udacity",["core","staticPages"]);angular.module("foundation",["user"]);angular.module("tasks",[]);angular.module("gateways",["utility","embedded"]);angular.module("widgets",["user","filters"]);angular.module("staticPages",["core"]);angular.module("user",["utility","gateways","embedded","foundation"]);angular.module("permissions",["ui.autocomplete"]);
...
function(b){a.serverError=b.error;a.saveDisabled=!1}):a.saveDisabled=!1}}}]);angular.module("account").controller("linkedAccountsTab",["$scope","$timeout","currentUser","users","google","facebook","normalizedSocialService","session","siteExperience",function(a,d,b,c,e,f,g,h,j){a.loading=!0;a.managed=j.managedAccount;a.accounts={google:null,facebook:null};var k=function(){a.serverSuccess="";a.serverError=""},i=function(b,c,e){c=g.normalizeAccount(b,c);c.id===a.user["_"+b+"_id"]||e?c.loggedIn=!0:c={loggedIn:!1};return a.accounts[b]=c},l=function(b){a.loading=!1;a.serverError=
b.error};a.getLinkedAccountTypes=function(){return _.keys(a.accounts)};a.disconnect=function(b){a.loading=!0;var e={key:a.user.key};e[b+"_id"]=null;c.put(e).then(function(c){k();a.loading=!1;a.user=c;"github"!==b&&(a.accounts[b]=null)},l)};a.disconnectSocial=function(b){a.loading=!0;var e={key:a.user.key},d;_.each(a.user.social_accounts,function(a,c){a.account_type===b&&(d=c)});a.user.social_accounts.splice(d,1);e.social_accounts=a.user.social_accounts;c.put(e).then(function(c){k();a.loading=!1;a.user=
c;a[b]=null},l)};a.connect=function(b){a.loading=!0;var e=g.normalizeAccountService(b),f=function(g,h){var j=function(a){401===a.code&&f(g-1,3*h)};0===g&&(j=l);d(function(){e.getAccount().then(function(e){var d={key:a.user.key};d[b+"_id"]=e.id;c.put(d).then(function(c){e=i(b,e,!0);k();a.loading=!1;a.user=c},l)},j)},h)},j=function(){f(3,500)};e.login().then(function(a){a=g.createSessionData(b,a);h.put(a).then(j,j)},l)};k();b.fetchFullUser().then(function(){a.loading=!1;a.user=b.getUser();_.each(a.user.social_accounts,
function(b){a[b.affiliate_program_key]=b});_.each(a.getLinkedAccountTypes(),function(a){g.normalizeAccountService(a).getAccount().then(function(b){i(a,b)})})},l)}]);angular.module("account").controller("myCertificatesTab",["$q","$scope","$sce","currentUser","users","nodes",function(a,d,b,c,e,f){d.loading=!0;d.verifiedCertificates=[];d.nodeMap={};d.serverError="";var g=c.getUser();d.getBadges=function(){return _.filter(g._badges,function(a){return"Course Mastery"===a.name&&0<a.highest_level})};a.all([e.getCertificates("me").then(function(a){d.verifiedCertificates=a;a=_.map(a,function(a){return a.node_ref.key});return f.batchGet(a,{projection:"catalog"}).then(function(a){_.extend(d.nodeMap,
...
""===a.confirmedNewPassword}]});f=c.check(f);a.passwordErrors=f.errorText;f.valid?(f={key:d.getUser().key,password:{old:a.currentPassword,"new":a.newPassword}},b.put(f).then(function(){a.serverSuccess="Password successfully updated!";e()},function(b){a.serverError=b.error;a.saveDisabled=!1})):a.saveDisabled=!1}}}]);angular.module("account").controller("subscriptionCancellation",["$scope","modal","$q","api","$location","customers","$window","recordActivity","dateTime","currentUser","$rootScope","cancellationFlow",function(a,d,b,c,e,f,g,h,j,k,i,l){a.cancelSubscription=l.cancelSubscription;a.navigateToPasswordTab=function(){e.path("/password");d.cancel()};a.passwordForm={};a.confirmSubscriptionCancellation=function(e,h,j){a.state="checking";c.post("/session/reauthentication",{password:h}).then(function(){g.dataLayer&&
g.dataLayer.push({subscriptionState:j,subscriptionId:e,event:"SubscriptionCancelled"});return f.removeSubscription("me",e)},function(){return b.reject("invalidPassword")}).then(function(){a.state=null;a.passwordForm={};d.confirm()},function(a){return b.reject(a||"cancelFailed")}).then(null,function(b){a.state=b})};a.survey={};a.surveyChoices=[{name:"The course requires more time than I expected",code:"tooMuchTime"},{name:"Due to unforeseen reasons I am unable to continue as planned",code:"unforeseenReasons"},
...
function(e,f,g,h,j){var k=["sessionExpired"],i=g.key;e.defaults.headers.common["X-Udacity-Client"]="udacity.com; version="+j;var l=function(c){var g={get:function(b,f,h){h=h||{};"yes"===window.localStorage.getItem("bustAllTheCaches")&&(f=f||{},f.cacheBusterValue=Math.random());h.params=a(f);var j=g.buildGetUrl(b,f),k=d();return 1600<j.length?(h.lengthy||console.warn("Url potentially too large for GET request; switching to POST. Set configuration option lengthy=true to suppress this warning."),delete h.params,
...
encodeURIComponent(d)+"/cards/"+encodeURIComponent(b)).then(function(a){return a.data})},graduate:function(d,b){return a.post("/customers/"+encodeURIComponent(d)+"/subscriptions/"+encodeURIComponent(b)+"/graduate").then(function(a){return a.data})}}}]);angular.module("gateways").factory("degrees",["api",function(a){return{migrateCohort:function(d,b,c,e){b={from_key:b,to_key:c,account_key:e};return a.post("/degrees/"+encodeURIComponent(d)+"/migrate_cohort",b).then(function(a){return a.data})}}}]);angular.module("gateways").factory("entityStore",["entityStoreHelper","$q",function(a,d){var b=function(a){return!_.isNull(a)&&!_.isUndefined(a)},c={setProjections:function(b,c){a.setProjections(b,c)},get:function(c,f,g,h,j){return h&&void 0!==h.depth?g(c,h).then(function(d){b(d)&&a.stash(c,f,j,d);return d}):d.when(a.getFilledEntity(c,f,j)).then(function(d){return d?d:g(c,h).then(function(d){b(d)&&a.stash(c,f,j,d);return d})})},batchGet:function(c,f,g,h,j){if(h&&void 0!==h.depth)return g(c,h).then(function(c){return c?
(_.each(_.filter(_.values(c),b),function(b){a.stash(b.key,f,j,b)}),c):{}});var k={},i=[];_.each(c,function(b){var c=a.getFilledEntity(b,f,j);c?k[b]=c:i.push(b)});return d.when(k).then(function(b){return 0<i.length?g(i,h).then(function(c){c&&_.each(_.filter(_.values(c),function(a){return a}),function(c){a.stash(c.key,f,j,c);b[c.key]=c});return b}):b})},standardMixin:function(a,b){var d=_.omit(b,"get","batchGet");d.__gateway=b;d.get=function(d,g){return g&&g.__skipCache?b.get(d,g):c.get(d,a,b.get,g,
...
encodeURIComponent(b)+"/enrollments",f).then(function(a){return a.data.modified_enrollments},function(){return a.reject("Failed to change enrollments")})},changeEnrollmentSettings:function(a,c,e){return d.post("/users/"+encodeURIComponent(c)+"/enrollments/"+encodeURIComponent(a)+"/settings",e)},getCertificates:function(a,c){return d.get("/users/"+encodeURIComponent(a)+"/certificates",c).then(function(a){return a.data.certificates})},getMemberships:function(a){return d.get("/users/"+encodeURIComponent(a)).then(function(a){return a.data.user._memberships})},
checkIfJabberIdIsStaff:function(a){return d.get("/users/account_for_jabber_id/"+encodeURIComponent(a)).then(function(a){if(a=a.data.user._memberships)return 0<_.filter(a,function(a){return-1<a.group_ref.key.indexOf("staff")}).length})},checkIfFirstChatVisit:function(a){return d.post("/users/"+a+"/tags",{tags_to_add:[{model:"Tag",namespace:"chat",tag:"introToChatSeen"}]})},addToEmailList:function(a,c,e,f){a={email_address:c,user_name:a,list_name:e};f&&(a.data=f);return d.post("/users/join_email_list",
...
a.courseKey}),function(a){return a[1].last_interaction_time}).reverse()[0][0]);b=c}else b=null;a.lastInteractedLessonKey=b}),j.getCertificates("me").then(function(a){i=a;_.each(i,function(a){var b=a.node_ref.key;_.has(l,b)?l[b].last_modification_time<a.last_modification_time&&(l[b]=a):l[b]=a})})]).then(function(){a.loading=!1;a.premiumAccess=h.hasPermissionTo(a.course,"view-paid-supplemental-media")||h.isCurrentEnrolled(a.enrollment,a.course)||h.isGraduated(a.enrollment,a.course)})});a.getCompletedLessonKeys=
...
"Group"===a.group_ref.ref&&e(a.group_ref.key)});a.ready=!0})}]);angular.module("overview").directive("nanodegreeEnrollmentActions",["cacheVersion","currentUser","groups","$location","dateTime","$window","gaqHelper","cookieJar","googleTagManager",function(a,d,b,c,e,f,g,h,j){return{restrict:"A",scope:{enrollmentActions:"="},templateUrl:"/templates/overview/nanodegree_enrollment_actions.html?"+a,controller:"enrollmentActionsHelper",link:function(a){var b=function(a){return{nd000:{start:"Mar. 4 at noon Eastern Time",end:"Mar. 10 at 11:59 pm Eastern Time"},nd001:{start:"Mar. 4 at noon Eastern Time",
end:"Mar. 10 at 11:59 pm Eastern Time"},nd002:{start:"Mar. 4 at noon Eastern Time",end:"Mar. 10 at 11:59 pm Eastern Time"},nd003:{start:"Mar. 4 at noon Eastern Time",end:"Mar. 10 at 11:59 pm Eastern Time"},nd004:{start:"Mar. 4 at noon Eastern Time",end:"Mar. 10 at 11:59 pm Eastern Time"}}[a]||{}},c=function(){j.pushTrackingEvent("nanodegreeAccess");a._goToClass()};a.getNanodegreeButtonInfo=function(){return a.isAccessible&&(a.isPaidEnrolled||a.isPaidGraduated)?{variant:"nanodegree is live, student is enrolled or graduated",
text:"Continue nanodegree",action:c}:a.isPaidEnrollable?a.isClassFull?a.isPaidWaitlisted?(h.exists("udacity_notify_me_clicked_recently")&&(j.pushTrackingEvent("notificationListJoin"),g.pushTrackingEvent("Waitlist","SignUp",a.courseKey),h.remove("udacity_notify_me_clicked_recently")),{variant:"nanodegree is full, student is waitlisted",statusMessage:"You will be emailed when enrollment opens "+b(a.courseKey).start,statusSuccess:!0}):{variant:"nanodegree is full, student not waitlisted, not enrolled",
text:"Notify me",sidenote:"when nanodegree re-opens",statusMessage:"<strong>Next enrollment opens "+b(a.courseKey).start+".</strong><br>You can start the program immediately after you enroll.",action:function(){h.set("udacity_notify_me_clicked_recently","yes",{expires:new Date((new Date).getTime()+6E5)});a._joinGroup(a.waitlistGroupKey)}}:{variant:"nanodegree live, student not enrolled",text:"Enroll in nanodegree",sidenote:"One week trial",statusMessage:"You can start the program immediately after you enroll. <br><strong>Enrollment closes "+
b(a.courseKey).end+".</strong>",action:function(){j.pushTrackingEvent("nanodegreeEnrollment");a._purchaseCourse()}}:{variant:"nanodegree not open for paid enrollment",sidenote:"This nanodegree is not yet available for enrollment"}};a=_.extend(a,a.enrollmentActions);a.productLaunchDate=new Date(a.productLaunchDateString);a.hasProductLaunched=a.enrollmentActions.isLaunched;a.isVIP=a.isAccessible&&(!a.isAvailable&&!a.isLaunched||a.canEditContent)&&a.getNanodegreeButtonInfo().action!==c;a.isFull=a.isClassFull&&
!(a.isPaidInterested||a.isPaidEnrolled||a.isPaidGraduated)}}}]);angular.module("overview").controller("nanodegreePortal",["$scope","cacheVersion","nodes","$window","currentUser","dateTime","api","$timeout","$q","productState","modal","fetchDiscourseLink",function(a,d,b,c,e,f,g,h,j,k,i,l){var m={};a.submissions={};a.submissionsFetched={};a.launchHangoutOnAir=function(a,b){i.activate(a).finally(function(){var a=$("#"+b),c=a.attr("src");a.attr("src","");a.attr("src",c)})};a.dismiss=function(){i.confirm()};var p=function(b){var b=b||0,c=_.find(a.submissionsFetched,
...
a.relatedCourses=_.pluck(a.nanodegree.overview_details.related_course_refs,"key");var d=a.relatedCourses;b.batchGet(d).then(function(b){_.each(d,function(c){var d=b[c];if(d){var e=null,f=d.catalog_entry;f&&f._image&&(e=f._image.serving_url);a.courses[c]={key:c,imageUrl:e,title:d.title||"Title for testing"}}})},function(a){console.warning("failed to fetch nodes"+a.data.error)});a.lessonStates={};a.lessonStates.completed={};a.lessonStates.showCheckmark={};a.lessonStates.visited={};j.all([b.state(a.nanodegreeKey,
"me").then(function(b){b&&_.each(b.completed_descendant_node_refs,function(b){a.lessonStates.completed[b.key]=!0})}),n(a.lessonKeys,a.lessonStates)]).then(function(){a.loadingMessage=null;p()});a.courseStates={};a.courseStates.completed={};a.courseStates.visited={};n(a.relatedCourses,a.courseStates)});l(a.nanodegreeKey).then(function(b){a.forumUrl=b})}]);angular.module("overview").directive("projectSubmissions",["cacheVersion","projects","fileQueue",function(a,d,b){return{restrict:"A",scope:{projectKey:"=",submissions:"=",submissionsFetched:"=",nanodegreeKey:"="},templateUrl:"/templates/projects/project_submissions.html?"+a,link:function(a){a.showLinkSubmission=!1;a.pickedFiles={};a.showSubmissionOptions={};a.sendToReviews=_.contains(["nd000","nd004"],a.nanodegreeKey);a.gradingUrl="https://review.udacity.com/#!/projects/"+a.projectKey;var e=function(){return d.getSubmissionsBySearch({node_key:a.projectKey,
student_key:"me",limit:100}).then(function(b){b=_.filter(b.submissions,function(a){return a.project_report&&a.project_report._completed&&"UNGRADED"===a.project_report._rating?!1:a._submission_link||a._submission_file||a._submission_text||a.project_report});a.submissions[a.projectKey]=_.sortBy(b,function(a){return a._creation_time}).reverse();b=a.submissions[a.projectKey];0===b.length?a.showSubmissionOptions=!0:(b=_.first(b).project_report,a.showSubmissionOptions=b&&b._completed&&"INCOMPLETE"===b._rating);
return a.submissions})},f=function(){a.showLinkSubmission=!1;a.submitting=!1;a.authorshipConfirmed=!1;a.error=!1},g=d.createSubmission;a.submitFile=function(f){a.error=!1;a.submitting=!0;return g(a.projectKey).then(function(g){return d.getSubmissionUploadUrl(g.key).then(function(d){return b.fileUploadTask(d,a.pickedFiles[f]).then(function(){a.pickedFiles[f]=null;return e()})})},function(){a.error=!0;a.submitting=!1})};a.showLinkSubmissionOption=function(){a.showLinkSubmission=!0};a.cancelSubmission=
function(b){a.pickedFiles[b]=null;f(b)};a.submitLink=function(b,d){a.submitting=!0;return g(a.projectKey,b,d).then(function(){return e()["finally"](f)},function(){a.error=!0;a.submitting=!1})};e().finally(function(){a.submissionsFetched[a.projectKey]=!0})}}}]);angular.module("overview").directive("courseEnrollmentActions",["cacheVersion","currentUser","groups","$location","dateTime","$window","adrollTracker","courseState","modal","googleTagManager",function(a,d,b,c,e,f,g,h,j,k){return{restrict:"A",scope:{enrollmentActions:"="},templateUrl:"/templates/overview/course_enrollment_actions.html?"+a,controller:"enrollmentActionsHelper",link:function(a){a.courseState=h;a.currentHighlightedFeature=0;a.freeCourseActionDefaultSectionTitle="Access Course Materials";
a.paidCourseActionDefaultSectionTitle="Enroll in Course";window.setInterval(function(){a.currentHighlightedFeature=(a.currentHighlightedFeature+1)%5;a.$apply()},4E3);a.getPaidCourseActionConfiguration=function(){if(a.isAccessible&&a.isPaidEnrolled)return{variant:"class is live, student is enrolled",text:"Go to class",hidePrice:!0,hideFeatures:!0,action:a._goToClass};if(a.hasProductLaunched)if(a.isPaidEnrollable){if(a.isClassFull&&a.isPaidWaitlisted)return{variant:"class is full, student is waitlisted",
statusMessage:"You will be be notified when course enrollment is open. You can access course materials now."};if(a.isClassFull)return{variant:"class is full, student not enrolled",text:"Notify me",note:"Get notified when enrollment is open",sectionTitle:"Enrollment is at capacity",hideFeatures:!0,action:function(){g.track(a.courseKey+"notify").then(function(){a._joinGroup(a.waitlistGroupKey)})}};if(a.hasProductLaunched)return{variant:"class is live, student not enrolled",text:"Start free trial",note:a.spotsRemaining&&
20>=a.spotsRemaining?"Only "+a.spotsRemaining+" seats left!":null,action:function(){k.pushTrackingEvent("paidCourseEnrollment");g.track(a.courseKey+"enroll").then(function(){a._purchaseCourse()})}}}else return{variant:"class not open for paid enrollment",hideSectionTitle:!0};else return{variant:"class not live",hideSectionTitle:!0}};var b=function(){k.pushTrackingEvent("freeCourseResume");a._goToClass()};a.getFreeCourseActionConfiguration=function(){return a.isPaidEnrolled?{variant:"class is live, student is paid enrolled",
hideSectionTitle:!0,placeholder:"You're all set, dive in!"}:a.isAccessible&&a.isFreeEnrolled?{variant:"class is live, student is enrolled",text:"Resume course materials",action:b}:a.isAvailable&&a.isEnrollable?{variant:"class is live, student not enrolled",text:"Access course materials",action:function(){k.pushTrackingEvent("freeCourseStart");g.track(a.courseKey+"free").then(function(){a._enrollInCourseware()})}}:a.isFreeInterested?{variant:"class not live, student is interested",hideSectionTitle:!0,
statusMessage:"You will be notified when the course is open."}:a.isEnrollable?{variant:"class not live, student not registered",text:"Notify me",note:"Get notified when the course is open",hidePrice:!0,hideFeatures:!0,action:function(){a._joinGroup(a.notificationGroupKey)}}:{variant:"error"}};a=_.extend(a,a.enrollmentActions);a.productLaunchDate=new Date(a.productLaunchDateString);a.hasProductLaunched=a.enrollmentActions.isLaunched;a.isVIP=a.isAccessible&&!a.isAvailable&&a.getFreeCourseActionConfiguration().action!==
b&&a.getPaidCourseActionConfiguration().action!==a._goToClass}}}]);angular.module("overview").directive("courseOverviewPage",function(){return{require:"overviewPage",controller:["$scope","courseState",function(a,d){a.courseState=d}]}});angular.module("overview").controller("enrollmentActionsHelper",["$scope","cacheVersion","currentUser","groups","$location","dateTime","$window",function(a,d,b,c,e,f,g){a._goToClass=function(){g.location.href="/course/viewer#!/c-"+g.encodeURIComponent(a.courseKey)};a._purchaseCourse=function(b){g.location.href="/checkout#!/0?products="+g.encodeURIComponent(b||a.productKey)};a._joinGroup=function(d){b.userIsLoggedIn()?(a.disableCoursewareAction=!0,c.addMember(d,"me").then(function(){a.disableCoursewareAction=
...
(a.fieldErrors=e.errorText,a.buttonDisabled=!1)};a.signInFacebook=function(){return k.getAuthResponse().then(function(a){return c.put({facebook:{signed_request:a.signedRequest}}).then(p,r)},n)};a.signInGoogle=function(){j.getToken(!1).then(function(a){return c.put({google:{token:a.code}}).then(p,r)},n)}}]);angular.module("account").controller("signUpForm",["$scope","$q","$sce","$timeout","session","users","currentUser","groups","validator","google","facebook","$location","safelyRedirect","gaqHelper","googleTagManager","retry","cookieJar",function(a,d,b,c,e,f,g,h,j,k,i,l,m,p,n,r,q){a._removeEmailCookie=function(){q.exists("emailAddressForSignUp")&&q.remove("emailAddressForSignUp",{path:"/"})};var t=function(){var b=function(){var b=a.initialSearch.next;b?m(b):m("/")},c=a.initialSearch.enroll,e=a.initialSearch.enroll_state||
"enrolled",f=a.initialSearch["join-group"],e=c?g.enroll(c,e,!0):d.when(!0),j=f?g.fetchFullUser().then(function(){return h.addMember(f,g.get("key"))}):d.when(!0),k,i;a._removeEmailCookie();k=c?p.pushTrackingEvent("SignUp","Enroll",c):p.pushTrackingEvent("SignUp","Direct");i=n.pushTrackingEvent("RegistrationCompleted");return e["finally"](function(){return j["finally"](function(){return k["finally"](function(){return i["finally"](b)})})})},u=function(c){a.errorMessage=b.trustAsHtml("We were unable to validate your account.");
return d.reject(c)},v=function(c){a.errorMessage=b.trustAsHtml("We were unable to access your account.");return d.reject(c)},s=function(c){409===c.data.status?(a.errorMessage=b.trustAsHtml("This account already exists, please sign in to continue."),a._removeEmailCookie()):a.errorMessage=b.trustAsHtml("Something went wrong and we're currently looking into it.");a.buttonDisabled=!1;return d.reject(c)};a.signUpEmail=function(){a.buttonDisabled=!0;a.errorMessage=null;a.fieldErrors=null;var b=j.check({email:{pretty:"your email address",
...
s):(a.fieldErrors=b.errorText,a.buttonDisabled=!1)};a.signUpFacebook=function(){return i.getAuthResponse().then(function(a){return i.getMeResponse().then(function(b){return f.registerUser({email_address:b.email,first_name:b.first_name,last_name:b.last_name,facebook:{signed_request:a.signedRequest},tos_accepted:!0}).then(t,s)},v)},u)};a.signUpGoogle=function(){k.getToken(!1).then(function(a){return e.put({google:{token:a.code}}).then(t,function(){return r(k.getMeResponse,5,100).then(function(a){return k.getToken(!0).then(function(b){return f.registerUser({email_address:a.email,
...
b){return b+" = "+a}),a.sort(),c.details=a.join(", "))}},f=function(a,f){c.key=a?a.key:f;c.name=f;if(a){c.imageLink=a.image_url||b[a.kind]||b.unknown;var j=d[a.kind];j?(c.details=null,j(a)):c.details="unknown kind: "+a.kind}else c.details="not found",c.imageLink=b.unknown;c.rows=c.name&&c.details?2:1};c.$watch("ref",function(d){null===d||void 0===d?(c.key=null,c.imageLink=null,c.name=null,c.details=null):d.kind&&d.key?f(d):(d=a.normalizeRef(d),d!==c.key&&(c.imageLink=b.unknown,c.name=null,c.details=
...
a.addEventListener("lessonChange",function(c){d.userIsLoggedIn()&&b.loadLibraries(["https://cdn.firebase.com/js/client/1.1.2/firebase.js"]).then(function(){var b=c.lesson.key;i&&i.off();var g=new Firebase("https://udacity.firebaseio.com");i=g.child("node-"+b);var k=i.child("active-users"),p=d.get("key");e.currentUsers={};k.on("value",function(a){e.currentUsers=a.val();m()});g.child("enabled").on("value",function(a){a.val()?(Firebase.goOnline(),e.revealChat=!0):(Firebase.goOffline(),e.revealChat=!1)});
a.addEventListener("courseChange",function(b){var c=_.findWhere(d.get("_enrollments"),{node_key:b.course.key});e.isPaidStudent=f.isCurrentEnrolled(c,b.course);g.child("courses").once("value",function(c){if(-1===_.values(c.val()).indexOf(b.course.key))Firebase.goOffline(),e.revealChat=!1;else{Firebase.goOnline();e.revealChat=!0;var d=k.push({name:l,userKey:p});d.onDisconnect().remove();a.addEventListener("lessonChange",function(){d.remove()},{removeOnDestructionOf:e})}})},{replayLast:!0,removeOnDestructionOf:e});
...
(i.push({sender:l,text:e.chatMessage,date:Firebase.ServerValue.TIMESTAMP}),e.chatMessage="")}};e.getUsernameColor=function(a){var b="#000000 #303030 #4E4E4E #666666 #767676 #999999 #B3B3B3 #294860 #307699 #1199C3 #8DC5D8 #F6A926 #F08C35 #E29791 #90CC83".split(" "),a=a.charCodeAt(a.length-1)%b.length;return b[a]};e.pingUser=function(a){e.chatMessage="@"+a.name};b=i.limit(100);b.once("value",function(a){e.numberOfUnreadMessages-=_.values(a.val()).length});b.on("child_added",function(a){s.push(a.val());
e.startCountingUnreadMessages&&(e.numberOfUnreadMessages+=1);e.chatHistory=s;m();_.defer(function(){n()})});e.lessonTitle=c.lesson.title})},{replayLast:!0,removeOnDestructionOf:e})}}}]);angular.module("user").factory("currentUser",["$log","$q","$rootScope","users","basicUser","gaqHelper","nodes","nodeUtility","userUtility","cohortsGateway",function(a,d,b,c,e,f,g,h,j,k){var i=angular.copy(e),l={get:function(a){return i[a]},getUser:function(){return i},getFullName:function(){return j.getFullName(i)},isStaff:function(){if(i._memberships)return 0<_.filter(i._memberships,function(a){return-1<a.group_ref.key.indexOf("staff")}).length},getEnrolledCourses:function(){l.get("_enrollments");
return _.chain(l.get("_enrollments")).filter(function(a){if(a&&"enrolled"===a.state)return!0}).map(function(a){return a.node_key}).value()},getGroupKeys:function(){l.get("_memberships");return _.chain(l.get("_memberships")).filter(function(a){return a.group_ref&&"Group"===a.group_ref.ref}).map(function(a){return a.group_ref.key}).value()},getEmail:function(){return j.getEmail(i)},setUser:function(a){angular.copy(angular.copy(a),i)},fetchFullUser:function(a){a=a?{cacheBust:Math.random()}:{};return c.get("me",
a).then(function(a){l.setUser(a);return a})},checkIfFirstChatVisit:function(){return c.checkIfFirstChatVisit("me")},fetchMemberships:function(){return c.getMemberships("me").then(function(a){return i._memberships=a})},fetchCohorts:function(){return l.fetchFullUser().then(function(a){return d.all(_.map(a._cohort_keys,function(a){return k.get(a)}))},function(b){a.error(b)})},userIsLoggedIn:function(){return i._registered},isEnrolledIn:function(a){return _.some(i._enrollments,function(b){return b.node_key===
a&&"enrolled"===b.state})},isEnrolledForCreditIn:function(a){var b=["151101088","150207572","151148782","151059885","151254701"];return void 0!==_.find(i._enrollments,function(c){var d=_.pluck(c.group_refs,"key");return c.node_key===a&&"enrolled"===c.state&&0<_.intersection(d,b).length})},isCourseEmailSubscribed:function(a){return 0<_.filter(i._enrollments,function(b){return b.node_key===a&&b.email_ok}).length},enroll:function(a,d,e){d=d||"enrolled";return c.setEnrollments("me",[a],d).then(function(a){i._enrollments=
i._enrollments||[];_.each(a,function(a){var b=_.find(i._enrollments,function(b){return b.node_key===a.node_key});b?i._enrollments[_.indexOf(i._enrollments,b)]=a:i._enrollments.push(a)});if(1===a.length){b.$broadcast("modifiedEnrollment",a);var c=d.slice(0,1).toUpperCase()+d.slice(1,-2),c=f.pushTrackingEvent("Enrollment",c,a[0].node_key);return e?c.then(function(){return a},function(){console.warn("Failed to push enrollment to Google Analytics");return a}):a}return a})},changeEnrollmentSettings:function(a,
b){return c.changeEnrollmentSettings(a,"me",b)},getEnrollmentsWithState:function(a){return _.filter(i._enrollments,function(b){return b.state===a})},setTag:function(a,b){var e=l.get("tags");if(_.findWhere(e,{namespace:a,tag:b}))return d.when(e);var f=e.concat([{namespace:a,tag:b}]);i.tags=f;return c.put({key:l.get("key"),tags:f}).catch(function(a){i.tags=e;return d.reject(a)})},removeTag:function(a,b){var e=l.get("tags");if(!_.findWhere(e,{namespace:a,tag:b}))return d.when(e);var f=_.reject(e,function(c){return c.namespace===
a&&c.tag===b});i.tags=f;return c.put({key:l.get("key"),tags:f}).catch(function(a){i.tags=e;return d.reject(a)})}};return l}]);angular.module("user").directive("userMenu",["currentUser","session",function(a,d){return{restrict:"A",controller:["$scope",function(){}],link:function(b){b.isLoggedIn=a.userIsLoggedIn;b.user=a.getUser;b.logOut=function(){d.terminate().then(function(){window.location.reload()})}}}}]);angular.module("user").factory("userUtility",[function(){return{getFullName:function(a){if(a)return a.first_name&&a.last_name?a.first_name+" "+a.last_name:a.first_name||a.last_name},getEmail:function(a){return a.email&&a.email.address}}}]);angular.module("utility").factory("adrollTracker",["$q","$timeout",function(a,d){return{track:function(b){var c={},e=a.defer();b&&(c.adroll_segments=b);try{window.__adroll.record_user(c)}catch(f){}d(function(){e.resolve(!0)},250);return e.promise}}}]);angular.module("utility").directive("animatedToggle",function(){return{restrict:"A",link:function(a,d,b){b.toggleTarget||console.error("You must provide a toggle-target with a selector for the child element to toggle");b.toggler||console.error("You must provide an element to trigger the toggle");var c=d.find(b.toggleTarget),a=d.find(b.toggler);c||console.error("Element to toggle ("+b.toggleTarget+") not found!");a||console.error("Element to trigger toggle ("+b.toggler+") not found!");$(a).on("click",
function(){$(c).toggle("fast")});window.location.hash.substring(1)===a.attr("id")?$(c).show():$(c).hide()}}});angular.module("utility").factory("callbackList",function(){return{create:function(){var a=[],d,b=!1,c={add:function(e,f,g){a.push(e);b&&f?(e(d),g&&c.remove(e)):g&&(e._removeOnFire=!0);return c},remove:function(b){for(var d=a.length,g=0;g<d;g++)a[g]===b&&(a.splice(g,1),g-=1);return c},fire:function(e){var f,g=_.clone(a),h=g.length;for(f=0;f<h;f++)try{g[f](e)}catch(j){console.error("Exception encountered in callback:"+j.message),console.error(g[f])}d=e;b=!0;for(f=0;f<a.length;f++)!0===a[f]._removeOnFire&&
...
identity:"Linux"}],j=function(a){if(null===b.browser||a)b.browser=e(g)||"An unknown browser";return b.browser},k=function(a){if(null===b.version||a)j(),b.version=f(navigator.userAgent)||f(navigator.appVersion)||"an unknown version";return b.version},i=function(a){if(null===b.os||a)b.os=e(h)||"an unknown OS";return b.os},l=function(a){if(null===b.cookies||a)document.cookie="cookietest=1",b.cookies=-1!==document.cookie.indexOf("cookietest="),document.cookie="cookietest=1; expires=Thu, 01-Jan-1970 00:00:01 GMT";
...
screen.colorDepth;return b.screenColorDepth};return a.eventEmission({browserSize:p,browserType:j,browserVersion:k,cookiesEnabled:l,flashEnabled:m,os:i,screenSize:n,screenColorDepth:r,ua:function(){return b.ua},language:function(){return b.lang},flushCache:c,browserStats:function(a){return{browser:j(a),version:k(a),os:i(a)}},getAllTech:function(a){a=!!a;j(a);k(a);i(a);l(a);m(a);p(a);n(a);r(a);return b}},["browserDetected"])}]);angular.module("utility").factory("userTrials",["mixin","currentUser","recordActivity","userTrialsHelper",function(a,d,b,c){var e={enroll:function(a,g){if("string"!==typeof a||"string"!==typeof g||""===a.trim()||""===g.trim())console.error("Trial key and treatment must both be non-empty strings.");else{var h=c.getEnrollments(d.get("key"));h[a]=g;c.saveEnrollments(d.get("key"),h);b.experimentAssignment(a,g);e.emitEvent("enrollment",{trial:a,treatment:g})}},unenroll:function(a){if("string"!==typeof a||
...
if(null!==f){var g=+f[1],h=+f[3],f=new Date(h,g-1,+f[2]);return(""+f.getFullYear()).slice(-2)===(h+"").slice(-2)&&f.getMonth()===g-1?a:d}return d}};return{check:function(a){var c,e={valid:!0,errorText:{}},f,g;for(g in a)if(a.hasOwnProperty(g)){c=a[g];for(var h=0;h<c.validators.length;h++)if(f=c.validators[h],"undefined"===typeof f.disabled||!1===f.disabled)if("function"===typeof d[f.type]){var j=c.value,k=c.pretty||g,i=d[f.type](j,k,f.args||[]);i.valid?f.hasOwnProperty("chained")&&(i={},i[g]={pretty:k,
value:j,validators:f.chained},f=this.check(i),!1===f.valid&&(e.valid=!1,e.errorText[g]=e.errorText[g]||[],e.errorText[g]=e.errorText[g].concat(f.errorText[g]))):(e.valid=!1,e.errorText[g]=e.errorText[g]||[],e.errorText[g].push("undefined"===typeof f.failMsg?i.errorText.charAt(0).toUpperCase()+i.errorText.slice(1):f.failMsg))}else console.warn("You tried to use a validator that doesn't exist! The name you tried to use was: "+f.type)}return e}}}]);angular.module("viewer").controller("contentGate",["$scope","modal","currentUser","$location",function(a,d,b,c){a.signUpAndReturnToCourse=function(a){window.location.href="/account/auth#!/signup?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(c.path())+"&enroll="+encodeURIComponent(a)};a.signInAndReturnToCourse=function(a){window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(c.path())+"&enroll="+encodeURIComponent(a)};a.enrollInCourse=function(c){a.enrollRequestPending=
...
function(){a.status="processing";a.quizReturn=null})}]);angular.module("viewer").controller("studentGoals",["$scope","firebaseGoals","studentGoals","users","currentUser","$routeParams","dateTime","userTrials","recordActivity","libraryLoader","cookieJar","lesson","$timeout",function(a,d,b,c,e,f,g,h,j,k,i,l,m){a.goal=b.getCurrent;a.goalKey=b.getCurrentKey;a.tutor=null;a.currentTime=g.getCurrentAsUTC;a.currentLesson=b.currentLesson;a.daysUntilDue=b.daysUntilCurrentGoalDue;a.checkGoal=function(){b.checkGoal(a.goal())};a.getGoalState=function(){var b=a.goal();
return"loading"===b?"loading":!b||!b.state?null:b.state.complete?"complete":b.target_time<g.getCurrentAsUTC()?"behind":"ongoing"};a.shown=!1;var p=function(){i.set("stGlDd",g.getCurrent().getTime()+432E5)};a.showGoal=function(){a.shown||j.observation("student_goal_open",1,{});a.shown=!a.shown};a.emailTutor=function(){j.observation("student_goal_coach_email",1,{})};d=function(a){a=1*a.slice(-1);return 4>a?"experimental":7>a?"control":"excluded"};"experimental"===(e.userIsLoggedIn()?h.getOrEnrollManually("student_goals",
d,[e.get("key")]):"excluded")&&c.getTutors("me",{node_key:f.courseKey}).then(function(c){if(_.isArray(c.tutor_assignments)){var d=_.find(c.tutor_assignments,function(a){return a.class_ref===f.courseKey});d&&k.loadLibraries(["//cdn.firebase.com/js/client/1.0.19/firebase.js"]).then(function(){b.init(e.get("key"),f.courseKey);a.tutor=c.accounts[d.tutor_account_ref];m(function(){if(!i.exists("stGlDd")||i.get("stGlDd")<g.getCurrent().getTime())a.shown=!0;p();l.addEventListener("morselChange",p)},1)})}})}]);angular.module("viewer").controller("videoPlayer",["$scope","lesson","youtubeVideoPlayer","viewerSettings",function(a,d,b,c){var e=!1;a.pauseVideo=function(){e&&b.pauseVideo()};a.obliterateVideo=function(){e&&b.destroyVideo()};a.playVideo=function(a,d){e?b.loadVideo(a.youtube_id,0):(b.attach(d.get(0),a.youtube_id,{autoPlay:c.get("autoPlay")?1:0}),e=!0)};b.addEventListener("apiReady",function(){d.addEventListener("pendingMorselChange",a.pauseVideo,{removeOnDestructionOf:a}).addEventListener("pendingLessonChange",
...
k.encodeURIComponent(e.key)+"&start="+k.encodeURIComponent(j.absUrl())},function(){c.showBanner=!1});l()}else c.showBanner=!1});c.goToPurchase=function(a){k.location.href="/checkout#!/0?products="+k.encodeURIComponent(_.first(a.catalog_entry.paid_class.class_product_refs).key)};c.cancelBanner=function(){d.set("cancel-side-panel-upsell",!0);c.showBanner=!1}}}}]);angular.module("viewer").directive("studentGoals",["cacheVersion","$document",function(a,d){return{restrict:"A",templateUrl:"/templates/viewer/partials/student_goals.html?"+a,controller:"studentGoals",link:function(a,c){var e=function(d){a.shown&&(!d||!c||!c[0].contains(d.target))&&a.$apply(function(){a.shown=!1})};d.on("click",e);a.$on("destroy",function(){d.off("click",e)})}}}]);angular.module("viewer").directive("supplementalMaterialsList",["cacheVersion",function(a){return{templateUrl:"/templates/viewer/partials/supplemental_materials_list.html?"+a}}]);angular.module("viewer").directive("viewerDiscussions",["forumService","lesson","$http","cacheVersion","siteExperience","fetchDiscourseLink",function(a,d,b,c,e,f){return{restrict:"A",templateUrl:"/templates/viewer/partials/viewer_discussions.html?"+c,controller:["$scope","$attrs","examMode",function(c,h,j){c.examMode=j;c.postsStartIndex=0;c.postsEndIndex=0;c.limitedAccess=e.limitedAccess;c.affiliateExperienceId=e.affiliateExperienceId;f().then(function(a){c.discourseCohortUrl=a});var k=h.numPerPage,
i=function(b,d){if(b&&d&&!c.limitedAccess)return c.seeAllCoursePostsUrl="http://forums.udacity.com/tags/"+b.key+"/#"+b.key,c.askAQuestionUrl="http://forums.udacity.com/questions/ask/?tag="+(d.containingExercise?d.containingExercise.key:d.key)+"#"+b.key,a.getPosts(d.containingExercise?d.containingExercise.key:d.key,0,k,b).then(function(a){c.posts=a;c.postsStartIndex=1+0*k;c.postsEndIndex=c.posts.length+0*k})};d.addEventListener("courseChange",function(a){var e=a.course.key;c.course=a.course;angular.isString(e)&&
"nd"===e.slice(e.length-2)?(c.isNanodegreeAssociatedCourse=!0,f(e).then(function(a){c.discourseCohortUrl=a})):"Degree"===a.course._semantic_type?(c.isNanodegree=!0,f(e).then(function(a){c.discourseCohortUrl=a})):"design101"===a.course.key?d.addEventListener("morselChange",function(a){if(a){c.posts=null;var a=window.encodeURIComponent(a.key),d=c.lesson.title,d=d.toLowerCase().replace(/[\?\!\:\&\,\(\)\^]/g,"").replace(" - ","-").replace(/\s+/g,"-").replace(/_/g,"-");b.get("https://design101-forum.udacity.com/subcategory/"+
a+".json?per_page=1").then(function(a){a.data.topic_list&&(c.posts=_.map(a.data.topic_list.topics,function(b){var c=null,d=_.find(a.data.users,function(a){if(b.posters[0])return a.id===b.posters[0].user_id});d&&(c=d.avatar_template);return{date:b.bumped_at,title:b.fancy_title,href:"https://design101-forum.udacity.com/t/"+b.slug+"/"+b.id,userPicUrl:c}}))});a="https://design101-forum.udacity.com/category/"+window.encodeURIComponent(d);c.askAQuestionUrl=a;c.seeAllCoursePostsUrl=a}},{replayLast:!0}):
"ud585"===a.course.key?(a=a.course.forum_slug,e=c.lesson.forum_slug,c.posts=null,c.seeAllLessonPostsUrl="http://discussions.udacity.com/c/"+a+"/"+e,c.askAQuestionUrl="http://discussions.udacity.com/c/"+a+"/"+e+"/create",c.isDiscourseSecondPilot=!0):(d.addEventListener("morselChange",function(a){c.morsel=a;c.posts=void 0;i(c.course,c.morsel)},{replayLast:!0}),i(c.course,c.morsel))},{replayLast:!0,removeAfterFired:!0})}]}}]);angular.module("viewer").directive("viewerFeedback",["validator","recordActivity","cacheVersion","lesson","lessonContentContext","techDetect","youtubeVideoPlayer",function(a,d,b,c,e,f,g){return{templateUrl:"/templates/viewer/partials/viewer_feedback.html?"+b,controller:["$scope","modal",function(b,c){b.form={feedbackReason:{value:null,validators:[{type:"required",failMsg:"Please select the type of feedback you wish to give."}]},feedbackExplanation:{value:null,validators:[{type:"javascript",args:[function(a,
...
code:"confusingContent",explanation:"e.g. course material is not explained correctly"}],i=[{name:"Instructions are unclear",code:"instructionsUnclear"},{name:"Inappropriate question",code:"inappropriateQuestion",explanation:"e.g. question is too easy/difficult"},{name:"Correct answer is not accepted",code:"correctAnswerNotAccepted"},{name:"Grading feedback is unhelpful",code:"gradingFeedbackUnhelpful"},{name:"Answer video is unclear",code:"answerVideoUnclear"}];b.showFeedbackDialog=function(){b.morselTitle=
b.morsel.title;"Video"===b.morsel.model?(g.pauseVideo(),b.feedbackOptions=k):b.feedbackOptions=i;c.activate("feedback")};var l=function(a){for(var a=_.pairs(a),b={},c=0;c<a.length;++c){var d=a[c][0],e=a[c][1];if(_.isObject(e))for(var e=_.pairs(l(e)),f=0;f<e.length;++f)b[d+"."+e[f][0]]=e[f][1];else void 0!==e&&null!==e&&(b[d]=e)}return b};b.submitFeedback=function(){b.formValidationResults=a.check(b.form);if(b.formValidationResults.valid){var k=[{context:e.getCurrent(),data:{model:"SurveyResponse",
survey_name:"LessonFeedback"}}],i={reason:b.form.feedbackReason.value,explanation:b.form.feedbackExplanation.value,techDetect:f.getAllTech(),currentUrl:window.location.href};"Video"===b.morsel.model&&(i=_.extend(i,{youtube:{availablePlaybackRates:g.getAvailablePlaybackRates(),currentPlaybackRate:g.getCurrentPlaybackRate(),currentTime:g.getCurrentTime()}}));i=l(i);k[0].data.items=_.map(_.pairs(i),function(a){return{name:a[0],content:a[1]}});d.send(k);c.confirm();b.form.feedbackReason.value=null;b.form.feedbackExplanation.value=
...
{morselId:h.key,comment:"We encountered an error. Please try submitting again."})})}else f(function(){_.reduce(h.widgets,function(a,b){return a&&l.answers[b.marker]===b.correct_value},!0)?l.emitEvent("correct",{morselId:h.key}):l.emitEvent("incorrect",{morselId:h.key})},10)}};return a.eventEmission(l,"ready submitting incorrect correct recorded error evaluation".split(" "))}]);angular.module("viewer").factory("isTrustedDomain",[function(){return function(a){return"https://www.udacity.com"===a||"http://localhost:8080"===a||"http://localhost:8087"===a||"https://assignments-dot-udacity-extras.appspot.com"===a}}]);angular.module("viewer").factory("lesson",["api","$q","mixin","lessonContentContext","typeForTag","freshContent",function(a,d,b,c,e,f){function g(a,b){var c=_.find(a,function(a){for(var c=b.length-1;0<=c;c--){var d=a.content_context[c];if(!d||d.node_ref.key!==b[c].node_key||d.tag!==b[c].tag)return!1}return!0});c&&(c=_.object(_.map(c.content_context,function(a){return[e[a.tag]+"Key",a.node_ref.key]})));return c}var h=null,j=null,k=null,i=null,l=null,m=null,p=null,n={announcePendingCourseChange:function(){n.emitEvent("pendingCourseChange",
{course:h})},changeCourse:function(b){var e=Math.round((new Date).getTime());p=e;d.all([a.get("/nodes/"+b+"?depth=1&fresh="+(!0===f)+"&required_behavior=view&projection=navigation"),a.get("/nodes/"+b+"/state?fresh=true")]).then(function(a){var d=a[0].data,f=a[1].data,i=a=null;if(e===p){h=d.references.Node[b];j=d.references.Node;h.steps_refs=_.reject(h.steps_refs,function(a){return!j[a.key]});if(d=_.find(f.nodestates,function(a){return a.node_ref.key===b}))(i=g(d.last_interactions,c.getCurrent()))&&
i.lessonKey&&(a=j[i.lessonKey]);n.emitEvent("courseChange",{course:h,lessons:_.map(h.steps_refs,function(a,b){var c=j[a.key];c.indexInCourse=b;return c}),lastLessonVisited:a})}},function(a){e===p&&a.status&&(403===a.status||404===a.status)&&n.emitEvent("coursePermissionDenied",b)})},announcePendingLessonChange:function(){n.emitEvent("pendingLessonChange",{lesson:k})},changeLesson:function(b){var e=Math.round((new Date).getTime());m=e;d.all([a.get("/nodes/"+b+"?depth=2&fresh="+(!0===f)+"&required_behavior=view&projection=classroom"),
a.get("/nodes/"+b+"/state?fresh=true")]).then(function(a){if(e===m){var d=a[0].data,f=a[1].data,a=null;k=d.references.Node[b];i=d.references.Node;k.steps_refs=_.reject(k.steps_refs,function(a){return!i[a.key]});_.each(k.steps_refs,function(a,b){var c=i[a.key];if(!c)throw Error("Expected a morsel but got "+JSON.stringify(c,2));"Exercise"===c.model&&(_.each(["lecture_ref","quiz_ref","answer_ref"],function(a){c[a]&&(c[a]=i[c[a].key],c[a]&&(c[a].containingExercise=c))}),_.each(c.tidbits,function(a){a.tidbit_ref=
i[a.tidbit_ref.key];a.tidbit_ref&&(a.tidbit_ref.containingExercise=c)}));i[a.key].indexInLesson=b});_.each(i,function(a){if("ImageFormQuiz"===a.model||"ProgrammingQuiz"===a.model)a.binCodes=[]});if(d=_.find(f.nodestates,function(a){return a.node_ref.key===b}))_.each(d.completed_descendant_node_refs,function(a){n.markMorselAsCompleted(a.key)}),(d=g(d.last_interactions,c.getCurrent()))&&d.morselKey&&(a=i[d.morselKey]);n.emitEvent("lessonChange",{lesson:k,morsels:_.map(k.steps_refs,function(a){return i[a.key]}),
lastMorselVisited:a})}},function(a){e===m&&a&&(a.status&&(403===a.status||404===a.status))&&n.emitEvent("lessonPermissionDenied",b)})},announcePendingMorselChange:function(a){n.emitEvent("pendingMorselChange",{from:l,to:a})},changeMorsel:function(a){l=i[a];n.emitEvent("morselChange",l)},changeTidbit:function(a){n.emitEvent("tidbitChange",{tidbit:a?i[a]:null})},markMorselAsCompleted:function(a){i[a]?i[a].completed=!0:console.error("Can't find morsel #"+a+" to mark it as completed")},updateProgressForMorsel:function(a,
b){i[a]&&i[b]?i[a].progressNodeId=b:console.error("Can't find container morsel #"+a+" or perhaps progress morsel #"+b+" to update progress")},updateLastSubmissionForMorsel:function(a,b){i[a]?i[a].lastSubmission=b:console.error("Can't find morsel #"+a+"to update activity")},mergeBinCodesForMorsel:function(a,b){i[a]&&(i[a].binCodes=_.union(i[a].binCodes,b))}};return b.eventEmission(n,"pendingCourseChange courseChange pendingLessonChange lessonChange pendingMorselChange morselChange tidbitChange lessonPermissionDenied coursePermissionDenied".split(" "))}]);angular.module("viewer").factory("lessonPrefetcher",["$http","$timeout","lesson",function(a,d,b){var c=null;b.addEventListener("courseChange",function(a){c=a.lessons},{replayLast:!0});b.addEventListener("courseChange",function(){b.addEventListener("lessonChange",function(b){var f=_.find(c,function(a){return a.key===b.lesson.key}).indexInCourse;if(0<f){var g=c[f-1].key;d(function(){a.get("/api/nodes/"+g+"?depth=2&fresh=false&required_behavior=view&projection=classroom")},1E4,!1)}if(0<=f&&f<c.length-
...
var i=null,l=null,m=function(b,c){var c=c||{},d={items:_.map(b,function(a){return{occurrence_time:c.timestamp||e.getCurrentAsUTC(),content_context:_.without(a.context,null,void 0,"none"),data:a.data}}),current_time:e.getCurrentAsUTC()},f="/api/activity";c.chain&&(f+="?related_chain=true");return a.post(f,d).then(function(a){200!==a.status&&(console.error("Error when trying to post activity. Response:"),console.error(a))})};c.addEventListener("videoPlay",function(a){i=a},{replayLast:!0});c.addEventListener("videoPause",
function(a){null!==i&&(null!==a&&void 0!==a)&&(l=a,m([{context:b.getCurrent(),data:{model:"PlayableView",start:i,stop:l}}]));i=l=null},{replayLast:!0});f.addEventListener("evaluation",k,{replayLast:!0});g.addEventListener("evaluation",k,{replayLast:!0});d.addEventListener("morselChange",function(a){if(!a.completed&&("Reading"===a.model||"EmbeddedFrame"===a.model))d.markMorselAsCompleted(a.key),m([{context:b.getCurrent(),data:{model:"CompletedExperience",completion_context:"Student viewed the node"}}])});
...
_.each(b,function(b){var c=_.find(a.parts,function(a){return a.marker===b.name});c&&(b.text=c.content)})}var j=null,k=null,i=null,l;e.addEventListener("morselChange",function(a){l=a},{replayLast:!0});var m=null,p=null,n=null,r=null,q={init:function(c){var d=Math.round((new Date).getTime()),f=c.key;j=d;p=g(c.initial_code_files);m=g(c.initial_code_files);n=c.key;r=c;c.hasOwnProperty("lastSubmission")?(h(c.lastSubmission,m),b.loadFiles(m).then(function(){q.emitEvent("ready")})):a.get("/nodes/"+c.key+
"/state").success(function(a){if(f===l.key&&d===j){var c=a.nodestates[0],a=c?c.last_submission_data:null,c=c?c.bin_codes:[];h(a,m);e.updateLastSubmissionForMorsel(n,a);e.mergeBinCodesForMorsel(n,c);b.loadFiles(m);q.emitEvent("ready")}}).error(function(a){console.error("Error fetching programming quiz progress data");console.error(a)})},submit:function(){var a=Math.round((new Date).getTime()),d=l.key;k=a;var g={operation:"GRADE",useIframe:l.evaluator&&"IFrameEvaluator"===l.evaluator.model,iframeUrl:l.evaluator&&
l.evaluator.external_uri};q.emitEvent("submitting",{morselId:n,submissionOptions:g});c.submitFiles(f(b.getAllFiles()),n,g).then(function(b){b=b.data;if(a===k&&d===l.key){e.mergeBinCodesForMorsel(d,b.evaluation.bin_codes);if(b.evaluation.passed||0<b.evaluation.score.points_earned)e.markMorselAsCompleted(d),r.containingExercise&&e.markMorselAsCompleted(r.containingExercise.key);b.evaluation.comment&&b.evaluation.comment.indexOf("est case input")&&(q.emitEvent("run",{morselId:d,result:{stdout:b.evaluation.comment,
stderr:null,images:null}}),b.evaluation.comment=b.evaluation.feedback?b.evaluation.feedback:"Please see detailed feedback in the output area below.");q.emitEvent("evaluated",{quiz:l,result:b.evaluation})}},function(a){q.emitEvent("evaluationError",a.data)})},run:function(){var a=Math.round((new Date).getTime()),d=l.key;i=a;c.submitFiles(f(b.getAllFiles()),n,{operation:"TEST",useIframe:l.evaluator&&"IFrameEvaluator"===l.evaluator.model,iframeUrl:l.evaluator&&l.evaluator.external_uri}).then(function(b){b=
b.data;if(a===i&&d===l.key){var c={};b.execution&&b.execution._output_attachments&&(c=b.execution._output_attachments,b=g(b.execution._ordered_file_attachments),c={stdout:c.stdout?c.stdout._content._plaintext:null,stderr:c.stderr?c.stderr._content._plaintext:null,images:_.filter(b,function(a){return"image"===a.kind})});q.emitEvent("run",{morselId:d,result:c})}},function(a){q.emitEvent("runError",a.data)})},reset:function(){b.loadFiles(p)}};return d.eventEmission(q,"submitting evaluated evaluationError run runError ready".split(" "))}]);angular.module("viewer").factory("studentGoals",["$q","$routeParams","firebaseGoals","nodes","lesson","currentUser","imageQuiz","programmingQuiz","dateTime",function(a,d,b,c,e,f,g,h,j){var k="loading",i,l={},m,p;b.addEventListener("updatedGoals",function(a){l=a;_.each(l,function(a){a.state=a.state||{complete:!1}});var b,d;_.each(a,function(a,c){if("visit"!==a.type&&(!b||a.created_time>b.created_time))d=c,b=a});k=(i=d||null)?l[i]:null;p=null;k&&(p=Math.floor(j.calculateDaysBetweenDates(j.getCurrent(),
new Date(k.target_time))),k.type&&"milestone"===k.type&&(k.milestoneData="loading",c.batchGet(k.config.milestones).then(function(a){k.milestoneData=a;n.checkGoal(k)})))},{replayLast:!0});e.addEventListener("lessonChange",function(a){m=a;n.checkGoal(k)},{replayLast:!0});g.addEventListener("correct",function(){n.checkGoal(k)},{replayLast:!0});h.addEventListener("evaluated",function(){n.checkGoal(k)},{replayLast:!0});var n={currentLesson:function(){return m},getCurrent:function(){return k},getCurrentKey:function(){return i},
checkGoal:function(a){if(!a||"milestone"!==a.type||!a.state||a.state.complete||!m)return!1;if(a.config.milestones[0]===m.lesson.key){var a=_.reduce(m.morsels,function(a,b){if("Exercise"===b.model||"ImageFormQuiz"===b.model||"ProgrammingQuiz"===b.model)a[0]+=1,b.completed&&(a[1]+=1),a[2].push(b.completed);return a},[0,0,[]]),c=_.reduce(a[2].slice(-4),function(a,b){return a+b},0);if(8<a[0]&&0.85<=a[1]/a[0]&&4===c||8>=a[0]&&a[0]===a[1])k.state.complete=!0,b.postCompletedUpdate(f.get("key"),d.courseKey,
i)}return!1},daysUntilCurrentGoalDue:function(){return p},init:function(a,c){b.init(a,c)}};return n}]);angular.module("viewer").factory("tidbits",[function(){return{isTidbitRelevant:function(a,d,b){return _.some(a.bin_code_groups,function(a){return(!b||a.bin_codes.length)&&_.every(a.bin_codes,function(a){return _.contains(d,a)})})},getTidbitViewerMode:function(a){return a?"tidbit-only":"quiz-only"}}}]);angular.module("viewer").constant("viewerPartialMap",{Video:{prefix:"video"},SketchVideo:{prefix:"video"},ProgrammingQuiz:{prefix:"programming_quiz",extension:!0},ImageFormQuiz:{prefix:"image_quiz"},Sketch:{prefix:"sketch"},EmbeddedFrame:{prefix:"iframe"},Reading:{prefix:"reading"}});angular.module("viewer").factory("viewerSettings",["mixin",function(a){var d={autoPlay:!0},b={set:function(a,e){if(d.hasOwnProperty(a)){var f=d[a];d[a]=e;b.emitEvent("change",{name:a,oldValue:f,newValue:e})}else console.error("can't set invalid setting: "+a)},get:function(a){if(d.hasOwnProperty(a))return d[a];console.error("Can't get invalid setting: "+a)}};return a.eventEmission(b,["change"])}]);angular.module("widgets").factory("github",["$q","libraryLoader","$rootScope","cookieJar",function(a,d,b,c){var e=d.loadLibraries(["base64"]),f={Accept:"application/vnd.github.raw","Content-Type":"application/json"},g={scopes:["repo"],note:"Udacity Issue Tracker",client_id:"bb427a8f7beb27b582e0",client_secret:"a014d61f827e1a761898d216ce44e23431237973"},h=null;(d=c.get("githubIssueTrackerToken"))&&(h=d);var j={setToken:function(a){null===a&&c.remove("githubIssueTrackerToken");h=a},hasToken:function(){return null!==
h},getAuth:function(d,g,l){var m=a.defer();e.then(function(){var a=angular.copy(f);a.Authorization="Basic "+Base64.encode(d+":"+g);l&&(a["X-GitHub-OTP"]=l);$.ajax("https://api.github.com/authorizations",{headers:a}).done(function(a){_.each(a,function(a){"bb427a8f7beb27b582e0"===a.app.client_id&&(j.setToken(a.token),c.set("githubIssueTrackerToken",a.token))});null!==h?m.resolve(a):j._postAuth(d,g,l).then(function(a){m.resolve(a)},function(a){m.reject(a)})}).fail(function(a){m.reject(a)}).always(function(){b.$apply()})});
return m.promise},_postAuth:function(c,d,h){var m=a.defer();e.then(function(){var a=angular.copy(f);a.Authorization="Basic "+Base64.encode(c+":"+d);h&&(a["X-GitHub-OTP"]=h);$.ajax("https://api.github.com/authorizations",{type:"POST",data:JSON.stringify(g),headers:a}).done(function(a){j.setToken(a.token);m.resolve(a)}).fail(function(a){m.reject(a)}).always(function(){b.$apply()})});return m.promise},getIssues:function(c,d){var e={};d&&(d.labels&&d.labels.join)&&(e.labels=d.labels.join(","));var g=
a.defer(),j=angular.copy(f);j.Authorization="token "+h;$.ajax("https://api.github.com/repos/udacity/"+c+"/issues",{headers:j,data:e,cache:!1}).done(function(a){g.resolve(a)}).fail(function(a){g.reject(a)}).always(function(){b.$apply()});return g.promise},postIssue:function(c,d){var e=a.defer(),g=angular.copy(f);g.Authorization="token "+h;$.ajax("https://api.github.com/repos/udacity/"+c+"/issues",{type:"POST",data:JSON.stringify(d),headers:g}).done(function(a){e.resolve(a)}).fail(function(a){e.reject(a)}).always(function(){b.$apply()});
return e.promise},updateIssue:function(c,d){var e=a.defer(),g=angular.copy(f);g.Authorization="token "+h;$.ajax("https://api.github.com/repos/udacity/"+c+"/issues/"+d.number,{type:"POST",data:JSON.stringify(d),headers:g}).done(function(a){e.resolve(a)}).fail(function(a){e.reject(a)}).always(function(){b.$apply()});return e.promise},getAssigneeOptions:function(c){var d=a.defer(),e=[],g=function(a){var j=angular.copy(f);j.Authorization="token "+h;$.ajax("https://api.github.com/repos/udacity/"+c+"/assignees?page="+
a+"&per_page=100",{type:"GET",headers:j}).done(function(b){e=e.concat(b);100>b.length?d.resolve(e):g(a+1)}).fail(function(a){d.reject(a)}).always(function(){b.$apply()})};g(1);return d.promise}};return j}]);angular.module("widgets").directive("staffIssues",["cacheVersion","github","lesson","lessonContentContext","$timeout","$window",function(a,d,b,c,e,f){var g=RegExp("(#### Github Issue Tracker Data Blob - DO NOT TOUCH ####)([\\s\\S]*?)(#### End of Github Issue Tracker Data Blob ####)","g");return{restrict:"A",templateUrl:"/templates/angular/staff_tools/staff_issues.html?"+a,link:function(a){a.issuePriorityOptions={p1:"OMG",p2:"HIGH",p3:"LOW"};a.labels=null;a.courseKey=null;a.showAddIssue=!1;a.currentMorselTitle=
null;var j=function(){d.getAssigneeOptions("course-issues").then(function(b){a.assigneeOptions=_.pluck(b,"login");a.assigneeOptions.unshift("")})};j();var k=function(){a.newIssue={title:"",body:"",priority:"p3"}};k();b.addEventListener("morselChange",function(b){a.labels=_.without(_.map(c.getCurrent(),function(a){return a.tag+a.node_key}),void 0,null);a.courseLabel=a.labels[0];a.lessonLabel=a.labels[1];a.morselLabel=_.last(a.labels);a.githubURLbase="https://github.com/udacity/course-issues/issues?labels=";
a.githubURL=a.githubURLbase+a.labels.join("%2C");a.currentMorselTitle=b.title;a.hasToken()&&(a.currentIssues=null,a.refreshIssues())},{replayLast:!0,removeOnDestructionOf:a});var i=function(b){b.body=b.body.replace(g,"");b.priority="p3";for(var c=[],d=0;d<b.labels.length;d++){var e=b.labels[d].name;e in a.issuePriorityOptions&&(b.priority=e);c.push(e)}b.labels=c;b.assignee=b.assignee?b.assignee.login:"";return b};a.refreshIssues=function(){d.getIssues("course-issues",{labels:[_.last(a.labels)]}).then(function(b){a.currentIssues=
_.map(b,i);a.showAddIssue=0===a.currentIssues.length?!0:!1})};a.closeIssue=function(b){confirm("Are you sure you want to close this issue?")&&(b.state="closed",a.postIssue(b))};a.addNewIssue=function(){var b={posted_from:{url:f.location.href,markdown:"["+a.currentMorselTitle+"]("+f.location.href+")"}},c=angular.copy(a.labels);c.push(a.newIssue.priority);a.newIssue.body+="\n#### Github Issue Tracker Data Blob - DO NOT TOUCH ####\n"+JSON.stringify(b,null,"\t")+"\n#### End of Github Issue Tracker Data Blob ####";
...
{projection:"image"}).then(function(b){b&&b._image_url&&a.partners[d].push({imageUrl:b._image_url,name:b.name})})})};a.fetchCards=function(b,c){return d.batchGet(b).then(function(d){return _.without(_.map(b,function(b){var e=!1,k=d[b];if(d[b]){var i=null,l=k.catalog_entry;l&&l._image&&(i=l._image.serving_url);c&&(k.affiliate_program_refs&&0<k.affiliate_program_refs.length)&&"udacity"!==k.affiliate_program_refs[0].key&&(e=!0,a._fetchAffiliate(k.affiliate_program_refs,b));return{key:b,servingUrl:i,
title:k.title,isNanodegree:"Degree"===k._semantic_type,showPartner:e&&c}}return null}),null)})};a._setLengthOfRow=function(a,b){return!b||6<b?6>=a?a:0===a%6?6:0===a%5?5:0===a%4?4:0===a%3?3:6:b};a.setUpCards=function(b,d,g,h,j){b?(a.lengthOfRow=a._setLengthOfRow(b.length,d),a.columnWidthClass="col-xs-"+a._calculateColumnWidth(a.lengthOfRow),a.cardImageSize=[1200/a.lengthOfRow,0],angular.isArray(h)&&2===h.length?a.cardImageSize=h:angular.isNumber(h)&&(a.cardImageSize=[h,0]),j?1===j?a.columnWidthClass=
"col-xs-12 col-sm-"+a._calculateColumnWidth(a.lengthOfRow):(a.cardsPerRowOnMobile=2,a.columnWidthClass="col-xs-6 col-sm-"+a._calculateColumnWidth(a.lengthOfRow)):a.columnWidthClass="col-xs-"+a._calculateColumnWidth(a.lengthOfRow),a.fetchCards(b,g).then(function(b){return a._generateRows(b,a.lengthOfRow)})["finally"](c)):(a.rows=null,a.columnWidthClass="",c())};a.finished=!1}]);angular.module("widgets").controller("courseFeatures",["$scope","modal",function(a,d){a.chooseFeature=function(b){a.chosenFeature=b};a.launchModal=function(b,c){a.projectTitle=c;a.chooseFeature(b);d.activate("features")}}]);angular.module("widgets").factory("guidedTour",["$q",function(a){var d=null,b=null,c=null;return{start:function(e){c&&c.reject();d=e;b=0;c=a.defer();return c.promise},goToExhibit:function(a){a=_.indexOf(d,_.findWhere(d,{id:a}));-1!==a&&(b=a)},next:function(){b<d.length-1?b+=1:(d=null,$().spotlight.clear(),c.resolve(),c=null)},previous:function(){0<b&&(b-=1)},getCurrentExhibitId:function(){return d&&d[b].id}}}]);angular.module("widgets").directive("guidedTourExhibit",["guidedTour",function(a){return{link:function(d,b,c){if(!c.guidedTourExhibit)throw Error("A guided tour placard must have a unique identifier!");b.addClass("guided-tour-exhibit-"+c.guidedTourExhibit);d.$watch(a.getCurrentExhibitId,function(a){a===c.guidedTourExhibit&&(b.spotlight({zIndex:1E4}),b.offset().top<$(window).scrollTop()?$("html, body").animate({scrollTop:b.offset().top-0.5*$(window).height()-0.5*b.outerHeight()},500):b.offset().top+
...
c.emitEvent("playbackRateChange",a.data)},onPlaybackQualityChange:function(a){var b=e.getAvailableQualityLevels();j[a.data]>j[g]?g=a.data:j[a.data]<j[b[0]]&&(g=a.data);c.emitEvent("playbackQualityChange",a.data)},onStateChange:function(a){a.data===YT.PlayerState.ENDED?c.emitEvent("videoEnd"):a.data===YT.PlayerState.PAUSED?c.emitEvent("videoPause",Math.floor(e.getCurrentTime())):a.data===YT.PlayerState.PLAYING&&c.emitEvent("videoPlay",Math.floor(e.getCurrentTime()))}}};h&&(b.playerVars.html5=1);e=
